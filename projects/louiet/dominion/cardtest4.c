...
int main() {
	...
	int retval = cardEffect(village, choice1, choice2, choice3, &test, handpos, &bonus);

	printf("--- Testing Card: %s ---\n", TESTCARD);
	newCards = 1; // draw 1 cards
	discarded = 1; // card discarded
	xtraActions = 2; // extra action

	int t0 = assertFunc(retval == 0, "card used"); //successful return from CardEffect
	int t1 = assertFunc(test.handCount[thisPlayer] == G.handCount[thisPlayer] + newCards - discarded, " +1 card");
	int t2 = assertFunc(test.deckCount[thisPlayer] == G.deckCount[thisPlayer] - newCards, " one less in deck");
	int t3 = assertFunc(test.coins == G.coins, "coins"); // number of coins unchanged by smithy
	int t4 = assertFunc(test.whoseTurn == G.whoseTurn, "player turn unchanged"); // player turn not changed
	int t5 = assertFunc(test.numActions == G.numActions + xtraActions, "actions"); //actions + 2
	int t6 = assertFunc(test.numBuys == G.numBuys, "buy phase" ); //number of buy remain the same
	int t7 = assertFunc(test.playedCardCount == G.playedCardCount, "card used"); //card discarded after use
