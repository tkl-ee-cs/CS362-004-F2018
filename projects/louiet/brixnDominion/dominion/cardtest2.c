...
int main() {
	...
	discarded = 1; // cards discarded (effect)
	unused = 1; //mine not used
	int b0 = assertFunc(retval == 0, "card used"); //successful return from CardEffect
	int b1 = assertFunc(test.handCount[thisPlayer] == G.handCount[thisPlayer] + newCards - discarded - unused, " +1 hand");
	...
	int c1 = assertFunc(test.handCount[thisPlayer] == G.handCount[thisPlayer] + newCards - discarded - unused, " +1 card");
