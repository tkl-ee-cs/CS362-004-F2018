...
// Description: card effects for playing Smithy
int playSmithy(int currentPlayer, int handPos, struct gameState *state) 
{
	int i = 0;

    //+3 Cards
    for (i = 0; i <= 3; i++)	// BUG: draws 4 cards
	{
...
// Description: card effects for playing Remodel
int playRemodel(int choice1, int choice2, int currentPlayer, int handPos, struct gameState *state)
{
	int i = 0;
...
// Description: card effects for playing Mine
int playMine(int choice1, int choice2, int currentPlayer, int handPos, struct gameState *state)
{
	...
	for (i = 0; i < state->handCount[currentPlayer]; i++)
	{
		...
		{
			discardCard(i, currentPlayer, state, 0);			
			//break;	// BUG: can discard more than one card of the same type
		}
	}
...
// Description: card effects for playing Adventurer
int playAdventurer(int currentPlayer, struct gameState *state)
{
	...
	while(drawntreasure<2){
		...
		if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
			...
			state->handCount[currentPlayer]++; // BUG: should decrement //this should just remove the top card (the most recently drawn one).
			z++;
		}
	}
...
// Description: card effects for playing Council Room
int playCouncilRoom(int currentPlayer, int handPos, struct gameState *state)
{
	...
	{
		// BUG: everyone draws a card, including current player
		/*if ( i != currentPlayer )
		{*/
  			drawCard(i, state);
		/*}*/
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case mine:
    	playMine(choice1, choice2, currentPlayer, handPos, state);
      	return 0;
			
    case remodel:
 		playRemodel(choice1, choice2, currentPlayer, handPos, state);
      	return 0;
