...
int main() {
	...
	int retval = cardEffect(great_hall, choice1, choice2, choice3, &test, handpos, &bonus);

	printf("---- Testing Card: %s ---\n", TESTCARD);
	newCards = 1; // draw card
	discarded = 1; // card discarded
	xtraActions = 1; // extra action
	...
	int t5 = assertFunc(test.numActions == G.numActions + xtraActions, "actions"); //actions + 1
